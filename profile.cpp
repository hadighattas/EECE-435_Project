    ...
    ui(new Ui::Profile)
{
    ...
    for (int i = 1; i < theUser.getGame1Scores().size(); i++)
        ui->textBrowser->append(theUser.getGame1Scores().at(i) + "\n");
    for (int i = 1; i < theUser.getGame2Scores().size(); i++)
        ui->textBrowser_2->append(theUser.getGame2Scores().at(i) + "\n");
    for (int i = 1; i < theUser.getGame3Scores().size(); i++)
    ...
    for(int i = 1; i < theUser.getGame1Scores().length(); i++)
        avg1 += theUser.getGame1Scores().at(i).toInt();
    if (theUser.getGame1Scores().length() == 1)
        avg1 = 0;
    else
        avg1 /= theUser.getGame1Scores().length() - 1;

    for(int i = 1; i < theUser.getGame2Scores().length(); i++)
        avg2 += theUser.getGame2Scores().at(i).toInt();
    if (theUser.getGame2Scores().length() == 1)
        avg2 = 0;
    else
        avg2 /= theUser.getGame2Scores().length() - 1;

    for(int i = 1; i < theUser.getGame3Scores().length(); i++)
        avg3 += theUser.getGame3Scores().at(i).toInt();
    if (theUser.getGame3Scores().length() == 1)
        avg3 = 0;
    else
        avg3 /= theUser.getGame3Scores().length() - 1;
